...
def parse_args():
    ...
    parser.add_argument(
        "--post-process", type=bool, default=False, help="Whether to post process feature map"
    )
...
def get_images(root_path):
    ...
    return [pre_process(image) for image in images], len(image_paths), image_paths
...
def demo(root_path):
    trt_engine_path = "/home/trace-dev/R-CenterNet/optimised_models/centernet_resnet_34_224.trt"

    engine = get_engine(1, "", trt_engine_path)
    context = engine.create_execution_context()
    inputs, outputs, bindings, stream = allocate_buffers(engine)
    print("Getting images")
    data, frame_count, image_paths = get_images(root_path)
    print("Starting inference")
    t = perf_counter()
    for i, data_point in enumerate(zip(data, image_paths)):
        (input, meta), path = data_point
        ...
        if args.post_process:
            ...
            ipdb.set_trace()
            detections = res
            detection_text_path = image_paths[i].split('.')[0] +'.txt'
            print('text path:', detection_text_path)
            detection_list_lists = []
            for detection in detections:
                print('runnn')
                detection_list = list(detection)
                dump_box_to_text(detection_list, detection_text_path)
    ...
    t1 = perf_counter()
    print("FPS: {}".format(frame_count / (t1 - t)))
