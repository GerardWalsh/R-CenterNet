...
class RegL1Loss(nn.Module):
  ...
  def forward(self, pred, mask, ind, target):
    ...
    # loss = nn.MSELoss(pred * mask, target * mask)
...
class CtdetLoss(torch.nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor): 
        ...
        # print('Target tensor reg', target_tensor['reg'])
        # print('Count of negative angles', torch.sum(pred_tensor['ang'] < 0))
        # print('Negative angles', pred_tensor[pred_tensor['ang'] < 0])


        hm_loss += self.crit(pred_tensor['hm'], target_tensor['hm'])
        # print('Target tensor: ', target_tensor['ang'].shape)
        # print('Target tensor: ', target_tensor['ang'].view(4, 1, 128))


        if True: # ang_weight > 0: # incorrect variable used in logic!
          ...
          #  pred = _transpose_and_gather_feat(pred_tensor['ang'], target_tensor['ind'])  
          # #  print('Predicted tensor shape: ', pred.shape)
          # #  print('Predicted tensor: ', pred.view(4, 1, 128))
          #  mask = target_tensor['reg_mask'].unsqueeze(2).expand_as(pred).float() 
          #  ang_loss += self.mse(pred*mask, target_tensor['ang']*mask)
        else:
          print('Negative angle!')
          # ang_weight = torch.abs(ang_weight)
          # pred_tensor['ang'] = _relu(pred_tensor['ang'])
          # ang_loss += self.crit_wh(pred_tensor['ang'], target_tensor['reg_mask'], target_tensor['ind'], target_tensor['ang'])  
          # ang_loss += self.mse(pred_tensor['ang']*target_tensor['reg_mask'], target_tensor['ind']*target_tensor['ang'])
